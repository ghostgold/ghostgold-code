...
public class FlowOpt
{
		...
		for(InstrList i = instr; i != null; i = i.tail){
			...
			if(!assem.equals("---")){
				...
				if(it != null){
					...
					if (it.temp != null){
						i.head = new MoveInstr("move `d0 `s0", exp.dst, it.temp);
					}
					else{
						Oper preexp = (Oper)it.instr.head;
						it.temp = new AtomicTemp();
						AtomicTemp predst = preexp.dst;
						preexp.setDst(it.temp);
						it.instr.tail = new InstrList(new MoveInstr("move `d0 `s0",predst, it.temp), it.instr.tail);
						i.head = new MoveInstr("move `d0 `s0", exp.dst, it.temp);
					}
				}
			}
			ArrayList<InstrListTemp> del = new ArrayList();
			if(i.head instanceof CallInstr){
	...
	public static void deadCodeElimination(ArrayList<BasicBlock> instrs){
		...
		for(BasicBlock block : instrs){
			...
			Set<AtomicTemp> live = liveness.liveAt(block);
			for(Instr i : reverseIns){
				i.dead = false;
			}
			for(Instr i: reverseIns){
				if(i instanceof BinInstr){
					BinInstr binop = (BinInstr)i;
				...
				else if (i instanceof MemInstr && i.opcode == MemInstr.LW){
		...
		while(flowChange){
			...
			for(BasicBlock block : instrs){
				...
				while(ins != null){
					...
					else if(ins.head instanceof MoveInstr){
						MoveInstr move = (MoveInstr)ins.head;
						removeACP(move.dst, newOut);
					}
					else if(ins.head instanceof CallInstr){
						CallInstr call =  (CallInstr)ins.head;
	...
	public static LinkedHashSet<CopyPair> copyLocalPropgation(BasicBlock block, LinkedHashSet<CopyPair> acp){
		...
		while(instrs != null){
			...
			else if(instrs.head instanceof MoveInstr){
				MoveInstr move = (MoveInstr)instrs.head;
				//	move.src = copyValue(move.src, acp);
				removeACP(move.dst, acp);
				if(move.dst != move.src)
					acp.add(new CopyPair(move));
			}
			else if(instrs.head instanceof CallInstr){
				CallInstr call =  (CallInstr)instrs.head;
				for(TempList dsts = call.def(); dsts != null; dsts = dsts.tail)
	...
	public static void removeACP(AtomicTemp t, LinkedHashSet<CopyPair> acp){
	...
	public static AtomicTemp copyValue(AtomicTemp t, LinkedHashSet<CopyPair> acp){
