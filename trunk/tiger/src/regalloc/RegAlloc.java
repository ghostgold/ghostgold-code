...
public class RegAlloc implements temp.TempMap{
	...
	public String tempMap(temp.AtomicTemp t){
	...
	ArrayList<BasicBlock> rewriteProgram(){
		...
		for(ListIterator<BasicBlock> b = blocks.listIterator(); b.hasNext(); ){
			...
			while(ins != null){
				if(ins.head instanceof MoveInstr){
					MoveInstr move = (MoveInstr)(ins.head);
					temp.AtomicTemp dst = move.dst;
					if( dst != null && spilledNodes.contains(interference.tnode(dst))){
						temp.AtomicTemp src = move.src;
						frame.Access mem = tempSpillMem.get(interference.tnode(dst));
						ins.head = new MemInstr("sw `s0 `i(`s1)", null , src, frame.FP(), mem.offSet(), frame, assem.MemInstr.SW);
					}
				}
				else if(ins.head instanceof Oper){
					Oper op = (Oper)(ins.head);
					temp.AtomicTemp def = op.dst;
					if( def != null && spilledNodes.contains(interference.tnode(def))){
						frame.Access mem = tempSpillMem.get(interference.tnode(def));
						temp.AtomicTemp spillTemp = new temp.AtomicTemp();
						spillTemp.setSpillCost(-1);
						op.setDst(spillTemp);
						ins.tail = new InstrList(new MemInstr("sw `s0 `i(`s1)",null, spillTemp, frame.FP(), 
														 mem.offSet(), frame, assem.MemInstr.SW),
												 ins.tail);
						ins = ins.tail;
					}
				}
				if(ins.tail != null){
					if(ins.tail.head instanceof MoveInstr){
						MoveInstr move = (MoveInstr)(ins.tail.head);
						temp.AtomicTemp src = move.src;
						if(src != null && spilledNodes.contains(interference.tnode(src))){
							temp.AtomicTemp dst = move.dst;
							frame.Access mem = tempSpillMem.get(interference.tnode(src));
							ins.tail.head = new MemInstr("lw `d0 `i(`s0)", dst, frame.FP(), null, mem.offSet(), frame,
													assem.MemInstr.LW);
						}
					}
					else if(ins.tail.head instanceof Oper){
						Oper op = (Oper)(ins.tail.head);
						temp.AtomicTemp left = op.left;
						temp.AtomicTemp spillLeft = left;
						if(left != null && spilledNodes.contains(interference.tnode(left))){
							frame.Access mem = tempSpillMem.get(interference.tnode(left));
							spillLeft = new temp.AtomicTemp();
							spillLeft.setSpillCost(-1);
							ins.tail = new InstrList(new MemInstr("lw `d0 `i(`s0)", spillLeft, frame.FP(), null, 
															 mem.offSet(), frame, assem.MemInstr.LW), ins.tail);
							ins = ins.tail;
						}
						temp.AtomicTemp right = op.right;
						temp.AtomicTemp spillRight = right;
						if(right != null && spilledNodes.contains(interference.tnode(right))){
							if(left != right){
								frame.Access mem = tempSpillMem.get(interference.tnode(right));
								spillRight = new temp.AtomicTemp();
								spillLeft.setSpillCost(-1);
								ins.tail = new InstrList(new MemInstr("lw `d0 `i(`s0)", spillRight, frame.FP(), null, 
																 mem.offSet(), frame, assem.MemInstr.LW), ins.tail);
								ins = ins.tail;
							}
	...
	void build(){
		...
		for(BasicBlock block : blocks){
			...
			Set<temp.AtomicTemp> live = liveness.liveAt(block);
			/*			while(l.hasNext()){
		...
		/*			System.out.println("liveout");
			...
			while(t != null){
				if(t.head instanceof MoveInstr){
					MoveInstr move = (MoveInstr)t.head;
